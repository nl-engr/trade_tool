...
class Executor extends Base {
  ...
  async doten(obj) {
    ...
    if (!this.isExchangeBitmex()) {
      let side = this.getOrderSide();
      if (side === null) {
        this.log('method getOrderSide 条件外 return');
        return;
      } else {
        this.log('method getOrderSide 条件OK！');
      }
    }
  ...
  async calculateQty() {
    ...
    this.log('total_collateral='+total_collateral);
  ...
  async getLastPrice(symbol) {
    ...
    // 5分以上昔の価格の場合、最新を取得
    if (moment.unix(lastTime).add(5, 'minutes') < moment().valueOf()) {
  ...
  readLatestOhlc() {
    ...
    this.log('ohlc='+[Number(ohlc['o']),Number(ohlc['h']),Number(ohlc['l']),Number(ohlc['c'])]);
  ...
  async createLimitOrder(side, amount, price) {
    ...
    if (this.isExchangeBitmex()) {
      ...
      // OHLCリストを取得
      let ohlcList = this.getOhlcList();
      if (ohlcList === null || ohlcList === undefined) return null; //有効でない場合はnullを返却

      let ohlc1 = ohlcList.slice(0,4); // 直近の15秒足*4
      let ohlc1mean = this.getOhlcMean(ohlc1);
      let ohlc2 = ohlcList.slice(4,8); // 1分以上前の直近の15秒足*4
      let ohlc2mean = this.getOhlcMean(ohlc2);
      let ohlc3 = ohlcList.slice(8,12); // 1分以上前の直近の15秒足*4
      let ohlc3mean = this.getOhlcMean(ohlc3);
      this.log('ohlc1mean='+ohlc1mean);
      this.log('ohlc2mean='+ohlc2mean);
      this.log('ohlc3mean='+ohlc3mean);

      // let judgePrice = this.getJudgePrice(ohlcList);
      // this.log('createLimitOrder:judgePrice='+judgePrice);

      // let ohlc = await readText.readLast('../data/ohlc.txt');
      let ohlc = this.readLatestOhlc();
      let open = Number.parseInt(ohlc[0]);
      let high = Number.parseInt(ohlc[1]);
      let low = Number.parseInt(ohlc[2]);
      let close = Number.parseInt(ohlc[3]);
      // let high = priceData[this.exchange.id][this.symbol].high;
      // let low = priceData[this.exchange.id][this.symbol].low;
      this.log('high='+high);
      this.log('low='+low);
      // let buyPrice = Number.parseInt(close + judgePrice/6);
      // let sellPrice = Number.parseInt(close - judgePrice/6);
      let buyPrice = Number.parseInt((high+close)/2 - (high-low)/6);
      let sellPrice = Number.parseInt((high+close)/2 + (high-low)/6);
      // 上げ続けている場合
      if (ohlc1mean > ohlc2mean && ohlc2mean > ohlc3mean ) {
        buyPrice = Number.parseInt((high+close)/2 + (high-low)/6);
      }
      // 下げ続けている場合
      if (ohlc1mean < ohlc2mean && ohlc2mean < ohlc3mean ) {
        sellPrice = Number.parseInt((high+close)/2 - (high-low)/6);
      }
  ...
  getOhlcMean(ohlc) {
    let ohlcMean = (Number(ohlc[0]['o']) + Number(ohlc[1]['o']) + Number(ohlc[2]['o']) + Number(ohlc[3]['o'])
      + Number(ohlc[0]['h']) + Number(ohlc[1]['h']) + Number(ohlc[2]['h']) + Number(ohlc[3]['h'])
      + Number(ohlc[0]['l']) + Number(ohlc[1]['l']) + Number(ohlc[2]['l']) + Number(ohlc[3]['l'])
      + Number(ohlc[0]['c']) + Number(ohlc[1]['c']) + Number(ohlc[2]['c']) + Number(ohlc[3]['c'])
    ) / 16;
    return ohlcMean
  }

  getJudgePrice(ohlcList) {
    let ohlc1 = ohlcList.slice(0,4); // 直近の15秒足*4
    let ohlc2 = ohlcList.slice(4,8); // 1分以上前の直近の15秒足*4
    let ohlc3 = ohlcList.slice(8,12); // 1分以上前の直近の15秒足*4

    let high1 = (Number(ohlc1[0]['h']) + Number(ohlc1[1]['h']) + Number(ohlc1[2]['h']) + Number(ohlc1[3]['h']))/4;
    let low1 = (Number(ohlc1[0]['l']) + Number(ohlc1[1]['l']) + Number(ohlc1[2]['l']) + Number(ohlc1[3]['l']))/4;
    let close2 = (Number(ohlc2[0]['c']) + Number(ohlc2[1]['c']) + Number(ohlc2[2]['c']) + Number(ohlc2[3]['c']))/4;
    // atr計算
    let atr1 =  Math.max(high1-low1,Math.abs(high1-close2),Math.abs(low1-close2));

    let high2 = (Number(ohlc2[0]['h']) + Number(ohlc2[1]['h']) + Number(ohlc2[2]['h']) + Number(ohlc2[3]['h']))/4;
    let low2 = (Number(ohlc2[0]['l']) + Number(ohlc2[1]['l']) + Number(ohlc2[2]['l']) + Number(ohlc2[3]['l']))/4;
    let close3 = (Number(ohlc3[0]['c']) + Number(ohlc3[1]['c']) + Number(ohlc3[2]['c']) + Number(ohlc3[3]['c']))/4;
    // atr計算
    let atr2 =  Math.max(high2-low2,Math.abs(high2-close3),Math.abs(low2-close3));
    // let judgePrice = (
    ...
    // ) / 16
    let judgePrice = Math.abs(atr2-atr1);
    return judgePrice;
  }

  getOhlcList() {
    this.log('getOhlcList');
    const dt = new Date();
    const formatted = dt.toFormat("YYYY-MM-DD");
    let ohlcFile = path.join(__dirname, '../data/'+formatted+'_ohlc.csv');
    if (!fs.existsSync(ohlcFile)) return null; // ファイルが存在しない場合
    const options = { columns: ['time','o','h','l','c','v']};
    const ohlcList = readCsvSync(ohlcFile, options).reverse();
    let ohlcTime = ohlcList[0]['time'];
    this.log('ohlcList.length='+ohlcList.length);
    this.log('formatted='+formatted);
    this.log('ohlcTime='+ohlcTime);
    // 取得したOHLCデータが1分以上過去の場合、足データがなく判定不能なため、例外的に実行OKとする
    if (moment() > moment(ohlcTime,'YYYY/MM/DD hh:mm:ss').add(2,'minutes')) {
      this.log('ohlcList moment()'+moment());
      this.log('moment1minutes='+moment(ohlcTime,'YYYY/MM/DD hh:mm:ss').add(1,'minutes'))
      return null;
    }
    // OHLCデータは正常だが、件数が判定に満たない場合falseを返す
    if (ohlcList.length < 12) {
      this.log('ohlcList.length < 12');
      return null;
    }
    // 有効の場合
    return ohlcList;
  ...
  getOrderSide(paramSide) {
    this.log('getOrderSide');
    // OHLCリストを取得
    let ohlcList = this.getOhlcList();
    if (ohlcList === null || ohlcList === undefined) return null; //有効でない場合はnullを返却
    let ohlc1 = ohlcList.slice(0,4); // 直近の15秒足*4
    ...
    if (ohlc1mean < ohlc2mean - judgePrice) {
      this.log('method getOrderSide '+CONST.SIDE.SELL);
      if (paramSide === CONST.SIDE.BUY) return null; // 逆方向の場合はなにもしない
      return CONST.SIDE.SELL;
    }
    if (ohlc1mean > ohlc2mean + judgePrice) {
      this.log('method getOrderSide '+CONST.SIDE.BUY);
      if (paramSide === CONST.SIDE.SELL) return null; // 逆方向の場合はなにもしない
      return CONST.SIDE.BUY;
    }
    return null;
  }
  ...
  async buyLogic(obj) {
    ...
    if (this.throughFlg) {
      this.log('スルー this.pos.side:add_qty'+this.pos.side +':'+this.add_qty);
      // this.dequeueById(obj.id); // キューから削除
    }
