...
class Executor {
  ...
  initLeverage() {
    ...
    // レバレッジ既存設定値がある場合読み込み
    if (this.state[this.symbol] && this.state[this.symbol].LV) {
      LEVERAGE[this.symbol] = this.state[this.symbol].LV;
    // 設定値がない場合は初期値をセット
    } else {
  ...
  async setLeverage(leverage) {
    ...
    if (result.value) { // 加工
      ...
      jsonRW.updateJson(this.symbol, 'LV', value.leverage, STATE);
    }
  ...
  async fetchAndCancelOpenOrder() {
    ...
    if (orders) {
      orders.filter(async(order, idx) => {
        // exchange ReNew
        this.exchange = new exchangeModule(this.env, this.exchange[this.strExchange]).exchange;
        // オープンOrderをキャンセル
  ...
  async fetchTickerLast() {
    ...
    let result = await this.order.fetchTicker(this.exchange, processor.getSymbolPair(this.symbol), 'last');
    if (result.e) {
      if (maxRetryCnt > 0) { 
        maxRetryCnt--;
  ...
  getStopLossPercentage() {
    ...
    log.log('percentage=' + CONST.BASIC_STOP_LOSS_PERCENTAGE/LEVERAGE[this.symbol]);
    return CONST.BASIC_STOP_LOSS_PERCENTAGE/LEVERAGE[this.symbol];
  }
  ...
  async adminCommandProcess(text) {
    ...
    if (this.command.infoStr) {
      message = processor.decorateWithBrackets(CONST.MSG_SYMBOL) + this.symbol + ' ' + processor.decorateWithBrackets(CONST.MSG_ENV) + ' '
        + processor.decorateWithInlineQuote(this.env) + processor.decorateWithBrackets(CONST.MSG_STATE) + ' ' + processor.decorateWithInlineQuote(this.state[this.symbol].MODE) + CONST.CRLF;
      ...
      message += '>' + processor.decorateWithBrackets(CONST.MSG_POS_SIDE) + ' ' + processor.decorateWithInlineQuote(this.pos.side)
        + processor.decorateWithBrackets(CONST.MSG_POS_QTY) + this.pos.currentQty + processor.decorateWithBrackets(CONST.MSG_LV) + (LEVERAGE[this.symbol]) + CONST.CRLF;
    ...
    // MODE判定
    log.log('MODE='+this.state[this.symbol].MODE);
    if (CONST.STR_MODE_OFF_ARRAY[0] === this.state[this.symbol].MODE) return;
