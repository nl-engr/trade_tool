...
class Executor {
  ...
  async fetchTotalBalance_bitflyer() {
    ...
    if (result.e) {
      if (maxRetryCnt > 0) { 
        maxRetryCnt--;
        log.log('result.e='+JSON.stringify(result.e));
        this.fetchTotalBalance_bitflyer();
      } else {
        // リトライ回数リセット
        maxRetryCnt = CONST.MAX_RETRY_CNT;
        return log.commonLogProcess(result, CONST.MSG_GET_BALANCE);
  ...
  async createMarketOrder(side, amount) {
    ...
    let result = await this.order.createMarketOrder(this.exchange, processor.getSymbolPair(this.symbol, this.exchange.id), side, this.amount);
    if (result.e) {
      if (maxRetryCnt > 0) { 
        maxRetryCnt--;
        log.log('result.e='+JSON.stringify(result.e));
        this.createMarketOrder(side, this.amount);
      } else {
        // リトライ回数リセット
        maxRetryCnt = CONST.MAX_RETRY_CNT;
  ...
  async setQty() {
    ...
    if (this.state[this.exchange.id][this.symbol].SIDE === this.side) {
