...
class Executor {
  ...
  async limitBuy(amount, price) {
    ...
    let result = await this.order.marketBuy(new exchangeModule(this.env).bitmex, processor.getSymbolPair(this.symbol), amount, price);
    if (result.value) { // 加工
      let value = result.value;
  ...
  async stoplossBuy() {
    ...
    if (result.value) { // 加工
      ...
      result.value = this.symbol+'-'+value.info.stopPx+'-'+value.amount;
    }
  ...
  async adminCommandProcess(text) {
    ...
    if (this.command.infoStr) {
      ...
      message += CONST.MSG_POSITION + CONST.CRLF;
      message += CONST.MSG_POS_SIDE + this.pos.side + CONST.CRLF;
      message += CONST.MSG_POS_QTY + this.pos.currentQty + CONST.CRLF;
      message += CONST.MSG_POS_ENTRY_PRICE + this.pos.avgEntryPrice + CONST.CRLF;
      message += CONST.MSG_POS_LIQUIDATION_PRICE + this.pos.liquidationPrice + CONST.CRLF;
      message += CONST.MSG_LV + (LEVERAGE[this.symbol]) + CONST.CRLF;
    ...
    // コマンド：BUY
      ...
      if (text) {
        // 指値/買い
        message += await this.limitBuy(new exchangeModule(this.env).bitmex, this.symbol, 1000, Number(text));
      } else {
        message = await this.buyLogic();
      }
    // コマンド：SELL
      ...
      if (text) {
        // 指値/買い
        message = await this.limitSell(new exchangeModule(this.env).bitmex, this.symbol, 1000, Number(text));
      } else {
        message = await this.sellLogic();
      }
    }
