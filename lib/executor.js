...
class Executor {
  constructor(env) {
    ...
    this.LEVERAGE = LEVERAGE ? LEVERAGE : CONST.LEVERAGE_10;    // レバレッジ設定
  ...
  async setLeverage(leverage) {
    ...
    let lv = leverage ? leverage : (this.symbol === CONST.SYMBOL_BTC) ? this.LEVERAGE : this.LEVERAGE_ALT;
    log.log('lv='+lv);
    let result = await this.order.setLeverage(this.exchange.bitmex, processor.getSymbolPairPosition(this.symbol), lv);
    if (result.value) { // 加工
