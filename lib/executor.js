...
class Executor {
  ...
  async command_change_leverage(text) {
    let result = await this.setLeverage(Number(text.replace(this.command.leverageStr, '')));
  ...
  async adminCommandProcess(text) {
    ...
    if (this.command.infoStr) {
      return await this.command_info();

    // コマンド：レバレッジ変更
    } else if (this.command.leverageStr) {
      return await this.command_change_leverage(text);
    }
  ...
  async buyLogic() {
    ...
    if (this.pos.side === null) {
      ...
      // まだ購入できる証拠金がある場合
      if (this.add_qty >= CONST.MIN_ADD_QTY) {
