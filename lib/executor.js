...
class Executor {
    ...
    if (result.value && result.value.length !== 0) {
      result.value.filter((position, idx) => {
        if (position.symbol === processor.getSymbolPairPosition(this.symbol)) {
          pos.side = (position.currentQty === 0) ? null : (position.currentQty > 0) ? CONST.SIDE.BUY : CONST.SIDE.SELL;
          pos.currentQty = (position.currentQty > 0) ? Math.round(position.currentQty).toFixed(2) : -Math.round(position.currentQty).toFixed(2);
    ...
    if (result.value && result.value.length !== 0) {
      ...
      result.value.filter((position, idx) => {
        if (CONST.BF_FX_BTC_JPY === position.product_code) {
          pos.side = position["side"] === CONST.STR_BUY_SIDE_ARRAY[0] ? CONST.SIDE.BUY : CONST.SIDE.SELL;
          ...
          priceList.push(position["price"]);
			...
			log.log('avgEntryPrice='+pos.avgEntryPrice);
    }
    result.value = this.setRetPosition(pos);
    return log.commonLogProcess(result, CONST.MSG_GET_POS_INFO).value;
  }
  ...
  async createStopOrder(side) {
    ...
    let params = this.makeStopParams(side);
    if (this.isExchangeBitmex()) {
  ...
  makeStopParams(side) {
    ...
    let stopPx = 0;
    let percent = (CONST.SIDE.BUY === side) ? 1+(this.getStopLossPercentage()/100) : 1-(this.getStopLossPercentage()/100);
    if (this.isExchangeBitmex()) {
  ...
  async buyLogic() {
    ...
    this.pos = await this.getPositions();
    // 損切りOrder
    result = await this.createStopOrder(CONST.SIDE.SELL);
    // メッセージ送信
    this.sendMessage(condition, result);
  }
  ...
  async sellLogic() {
    ...
    this.pos = await this.getPositions();
    // 損切りOrder
    result = await this.createStopOrder(CONST.SIDE.BUY);
    // メッセージ送信
    this.sendMessage(condition, result);
  }
}
