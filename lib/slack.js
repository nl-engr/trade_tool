...
rtm.on('message', async(event) => {
  ...
  // Botアラート受信時
    ...
    // stateファイル最新化
    updateStateSide(symbol, sign);
  } 
});

function changeEnv(event, text) {
  let changeEnvFlg = false;
  if ((text === CONST.ENV.TEST && env === CONST.ENV.PROD) || (text === CONST.ENV.PROD && env === CONST.ENV.TEST)) {
    // stateファイル読み込み
    let state = readState();
    env = text;
    state.ENV = env;
...
function judgeCommand(text, obj, event) {
  ...
  CONST.STR_BUY_SIDE_ARRAY.forEach((str, idx) => {
    ...
    if (matchedIndex !== -1) {
      ...
      commandFlg = true;
      // stateファイル最新化
      updateStateSide(obj.symbol, 'LONG');
    }
  });
  // コマンド(SELL)を判定
  CONST.STR_SELL_SIDE_ARRAY.forEach((str, idx) => {
    ...
    if (matchedIndex !== -1) {
      ...
      commandFlg = true;
      // stateファイル最新化
      updateStateSide(obj.symbol, 'SHORT');
    }
  });
  return commandFlg;
}
...
function writeState(state) {
  // stateファイル書き込み
  fs.writeFileSync(STATE, JSON.stringify(state, null, '    '));
}

function updateStateSide(symbol, sign) {
  // stateファイル読み込み
  let state = readState();
  if (!state[symbol]) state[symbol] = {};
  state[symbol].SIDE = sign;
  // stateファイル書き込み
  writeState(state);
}
