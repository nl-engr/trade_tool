...
rtm.on('message', async(event) => {
  ...
  if (adminModeFlg) {
    ...
    executor = new executorModule(env, rtm, event);
    // adminコマンド実行
  ...
  // Botアラート受信時
    ...
    let executor = new executorModule(env, rtm, event);
    // doten処理実行
    await executor.doten(symbol, sign);
  } 
});
...
function changeEnv(event, text) {
  ...
  if (text === CONST.ENV_TEST && env === CONST.ENV_PROD) {
    env = CONST.ENV_TEST;
    rtm.sendMessage('環境変更 '+processor.decorateWithInlineQuote(CONST.ENV_PROD)+' ⇛ '+processor.decorateWithInlineQuote(CONST.ENV_TEST), event.channel);
    changeEnvFlg = true;
  } else if (text === CONST.ENV_PROD && env === CONST.ENV_TEST) {
    env = CONST.ENV_PROD;
    rtm.sendMessage('環境変更 '+processor.decorateWithInlineQuote(CONST.ENV_TEST)+' ⇛ '+processor.decorateWithInlineQuote(CONST.ENV_PROD), event.channel);
    changeEnvFlg = true;
  }
  return changeEnvFlg;
}
  ...
  try {
    ...
    if (event.user === CONFIG.slack.admin_user) {
      ...
      if (event.text.indexOf('admin') !== -1) {
        ...
        msg = adminModeFlg ? processor.decorateWithInlineQuote(CONST.MSG_ADMIN_ON) : processor.decorateWithInlineQuote(CONST.MSG_ADMIN_OFF);
      ...
      // adminモードかつ「pause」文字列を含んでいる場合
        ...
        msg = pauseFlg ? processor.decorateWithInlineQuote(CONST.MSG_PAUSE_ON) : processor.decorateWithInlineQuote(CONST.MSG_PAUSE_OFF);
        adminCommandFlg = true;
        rtm.sendMessage(msg, event.channel);
      }
    }
