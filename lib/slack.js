...
const cryptowatchModule = require(path.join(__dirname, 'cryptowatch.js'));

const log = new logModule();
const Order = new orderModule();
const Cryptwatch = new cryptowatchModule();
...
async function stopProcess() {
  ...
  // adminコマンド実行
  await executor.adminCommand(obj);
}
...
new CronJob('0 59 4,12,20 * * *', async() => {
  log.log('*** MEX SWAPTIME cron RUNNING ***');
  // チャンネル情報があればセット
  let cnl = (channel) ? channel : CONFIG.slack.channel;
  ...
  // ポジションクローズ
  await executor.command_close(obj);
}, null, true);

// // 価格データ更新
// new CronJob('*/5 * * * * *', async() => {
//   log.log('*** 5s cron RUNNING ***');
//   // stateファイル読み込み
//   state = jsonRW.readJson(STATE);
//   // チャンネル情報があればセット
//   let cnl = (channel) ? channel : CONFIG.slack.channel;
...
//     // let price = await Cryptwatch.getPrice(CONST.SYMBOL_BTC, exchange);
//     let obj = {};
//     obj.exchange = exchange;
//     obj.symbol = CONST.SYMBOL_BTC;
//     obj.env = state[obj.exchange].ENV;
//     obj.channel = cnl;
//     let executor = new executorModule(rtm, obj);
//     let result = await executor.fetchTickerLast();
//     if (result.e) {
//       jsonRW.updateJson_state(exchange, CONST.SYMBOL_BTC, 'price', null, PRICE);
//     } else {
//       log.log(exchange +':'+lastPrice);
//       jsonRW.updateJson_state(exchange, CONST.SYMBOL_BTC, 'price', lastPrice, PRICE);
//     }
//   });
// }, null, true);

new CronJob('*/10 * * * * *', async() => {
  log.log('*** 10s cron RUNNING ***');
  // 価格更新
...
new CronJob('*/30 * * * * *', async() => {
  log.log('*** 30s cron RUNNING ***');
  // バランスを取得する
  await fetchAndUpdateTotalBalance();
  // 定期巡回
  await patrol();
}, null, true);
...
async function fetchAndUpdatePrice() {
  log.log('method fetchAndUpdatePrice');
  Object.keys(CONST.EXCHANGE).forEach(async(key, idx) => {
    let exchange = CONST.EXCHANGE[key];
    if (CONST.EXCHANGE.BITMEX === exchange) { // TODO
      return;
    }
    // let ohlc = await Cryptwatch.getOHLC(CONST.SYMBOL_BTC, exchange);
    // jsonRW.updateJsonOhlc(exchange, CONST.SYMBOL_BTC, ohlc, PRICE);
    let price = await Cryptwatch.getPrice(CONST.SYMBOL_BTC, exchange);
    jsonRW.updateJson_state(exchange, CONST.SYMBOL_BTC, 'price', price, PRICE);
    jsonRW.updateJson_state(exchange, CONST.SYMBOL_BTC, 'lastTime', moment().unix(), PRICE);
  });
}

// 現在の保有BTC総量を取得
async function fetchAndUpdateTotalBalance() {
  log.log('method fetchAndUpdateTotalBalance');
  // チャンネル情報があればセット
  let cnl = (channel) ? channel : CONFIG.slack.channel;
  // stateファイル読み込み
  state = jsonRW.readJson(STATE);
  Object.keys(CONST.EXCHANGE).forEach(async(key, idx) => {
    let exchange = CONST.EXCHANGE[key];
    let result = null;
    let total_collateral = null;
    if (CONST.EXCHANGE.BITMEX === exchange) {
      log.log('fetchAndUpdateTotalBalance bitmex');
      let obj = {};
      obj.exchange = exchange;
      obj.symbol = CONST.SYMBOL_BTC;
      obj.env = state[obj.exchange].ENV;
      obj.channel = cnl;
      let executor = new executorModule(rtm, obj);
      result = await Order.fetchBalance(executor.exchange);
      if (result.value) { // 加工
        total_collateral = result.value[CONST.SYMBOL_BTC].total;
        log.log('exchange='+exchange+' total_collateral='+total_collateral);
        jsonRW.updateJson_state(exchange, CONST.SYMBOL_BTC, 'total_collateral', total_collateral, BALANCE);
      }
    } else if (CONST.EXCHANGE.BITFLYER === exchange) {
      log.log('fetchAndUpdateTotalBalance bitflyer');
      let obj = {};
      obj.exchange = exchange;
      obj.symbol = CONST.SYMBOL_BTC;
      obj.env = state[obj.exchange].ENV;
      obj.channel = cnl;
      let executor = new executorModule(rtm, obj);
      result = await Order.getCollateral(executor.exchange);
      if (result.value) { // 加工
        // let open_position_pnl = result.value.open_position_pnl;
        total_collateral = (result.value.collateral + result.value.open_position_pnl);
        log.log('exchange='+exchange+' total_collateral='+total_collateral);
        jsonRW.updateJson_state(exchange, CONST.SYMBOL_BTC, 'total_collateral', total_collateral, BALANCE);
      }
    }
...
async function reDoten(obj) {
  log.log('method reDoten');
  log.log('reDoten obj='+JSON.stringify(obj));
  let executor = new executorModule(rtm, obj);
  // かつMODEがONの場合、直近のSIDEを再オーダー
  if (state[obj.exchange][obj.symbol]) {
    if (CONST.MODE.ON === state[obj.exchange][obj.symbol].MODE) {
      obj.side = state[obj.exchange][obj.symbol].SIDE;
      // obj.rd = CONST.MSG_REDOTEN;
      // rtm.sendMessage(CONST.MSG_REDOTEN, obj.channel);
      executor.doten(obj);
    }
  }
}

async function revaluateStopPx(obj) {
  log.log('method revaluateStopPx');
  log.log('revaluateStopPx obj='+JSON.stringify(obj));
  let executor = new executorModule(rtm, obj);
  // ポジション取得
  await executor.calculateQty();
  // ノーポジの場合
  ...
  if (executor.current_position_qty < 0.1) {
    return await executor.command_close(obj);
  }
...
async function main(obj) {
  ...
  // MODE:OFFの場合リターン
  if (CONST.MODE.OFF === state[obj.exchange][obj.symbol].MODE) return;
  let executor = new executorModule(rtm, obj);
  // orderIDが存在(注文済)の場合
  // if (obj.orderId) {
  ...
  //     await executor.command_info();
  //   }
  // }
  obj.side = (obj.command) ? obj.command.toLowerCase() : null;
  // BOT投稿
  if (obj.bot_id !== undefined) {
    // doten処理実行
...
function judgeAdminCommand(obj) {
  log.log('method judgeAdminCommand');
  try {
...
function setObj(obj) {
  ...
  setNumber(obj); // 数値設定
  log.log('setObj obj='+JSON.stringify(obj));
}
...
function stateChange(obj) {
  log.log('method stateChange');
  let stateChangeFlg = false;
  // 環境変更判定と変更
  if (CONST.ENV.TEST === obj.text || CONST.ENV.PROD === obj.text) {
    env = obj.text;
    jsonRW.updateJson_state(obj.exchange, null, 'ENV', obj.text, STATE);
